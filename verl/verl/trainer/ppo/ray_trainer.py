...
from verl.trainer.ppo.agentic_extensions import (
    CurriculumConfig,
    CurriculumSampler,
    apply_judge_reward,
    b_ndsr_metrics,
...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        self.curriculum_sampler = CurriculumSampler(CurriculumConfig.from_env())
    ...
    @staticmethod
    def _task_ids(batch: DataProto) -> list[int] | None:
        values = batch.non_tensor_batch.get("task_id")
        if values is None:
            raw_extra = batch.non_tensor_batch.get("extra_info")
            if raw_extra is not None:
                extracted = []
                for item in raw_extra:
                    if isinstance(item, dict) and item.get("task_id") is not None:
                        extracted.append(item["task_id"])
                values = extracted if len(extracted) == len(batch.batch) else None
        if values is None:
            return None
        try:
    ...
    def _apply_curriculum(self, batch: DataProto, metrics: dict[str, float]) -> DataProto:
        if not self.curriculum_sampler.config.enabled:
            return batch
        task_ids = self._task_ids(batch)
        if not task_ids:
            return batch
        chosen, cur_metrics = self.curriculum_sampler.choose(task_ids, len(task_ids), self.global_steps)
        positions: dict[int, list[int]] = defaultdict(list)
        for index, task_id in enumerate(task_ids):
            positions[task_id].append(index)
        rng = np.random.RandomState(self.global_steps + self.curriculum_sampler.config.seed)
        indices = [positions[task_id][int(rng.randint(len(positions[task_id])))] for task_id in chosen]
        metrics.update(cur_metrics)
        return batch.slice(indices)

    def _apply_b_ndsr(self, batch: DataProto, metrics: dict[str, float]) -> DataProto:
        if os.getenv("B_NDSR_ENABLED", "").strip().lower() not in {"1", "true", "yes", "on"}:
            return batch
        uids = np.asarray(batch.non_tensor_batch.get("uid", []), dtype=object)
        scores = batch.batch["token_level_scores"].sum(-1).detach().cpu().tolist()
        keep: list[int] = []
        plans = []
        for uid in dict.fromkeys(uids.tolist()):
            group = [i for i, value in enumerate(uids) if value == uid]
            plan = plan_b_ndsr([scores[i] for i in group])
            plans.append(plan)
            keep.extend(group[: plan.selected_rollouts])
        metrics.update(b_ndsr_metrics(plans))
        metrics.update(jass_metrics(batch))
        if not keep or len(keep) == len(uids):
            return batch
        keep.sort()
    ...
    def _generate_b_ndsr_batch(self, batch: DataProto, gen_batch: DataProto):
        ...
        for round_size, increment in ((4, 4), (6, 2), (8, 2), (10, 2), (12, 2)):
            ...
            prompt_indices = list(range(prompt_count)) if round_size == 4 else active
            round_gen = gen_batch.slice(prompt_indices).repeat(repeat_times=repeat, interleave=True)
            round_output = self.async_rollout_manager.generate_sequences(round_gen)
            round_base = batch.slice(prompt_indices).repeat(repeat_times=repeat, interleave=True)
            round_batch = round_base.union(round_output)
            if "response_mask" not in round_batch.batch:
                round_batch.batch["response_mask"] = compute_response_mask(round_batch)
            collected.append(round_batch)

            candidate = DataProto.concat(collected)
            raw_scores, _ = extract_reward(candidate)
            candidate.batch["token_level_scores"] = raw_scores
            candidate_uids = np.asarray(candidate.non_tensor_batch["uid"], dtype=object)
            next_active = []
        ...
        final_batch = DataProto.concat(collected)
        if "response_mask" not in final_batch.batch:
            final_batch.batch["response_mask"] = compute_response_mask(final_batch)
        raw_scores, reward_extra_infos = extract_reward(final_batch)
        final_batch.batch["token_level_scores"] = raw_scores
        return final_batch, reward_extra_infos
    ...
    def fit(self):
        ...
        for epoch in range(current_epoch, self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                batch = self._apply_curriculum(batch, metrics)
                ...
                agentic_bndsr = os.getenv("B_NDSR_ENABLED", "").strip().lower() in {"1", "true", "yes", "on"}
                gen_batch_output = None if agentic_bndsr else gen_batch.repeat(repeat_times=rollout_n, interleave=True)
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("gen", timing_raw, color="red"):
                        if agentic_bndsr:
                            batch, reward_extra_infos_dict = self._generate_b_ndsr_batch(batch, gen_batch)
                            reward_tensor = batch.batch["token_level_scores"]
                            metrics.update(b_ndsr_metrics([
                                plan_b_ndsr(batch.batch["token_level_scores"][
                                    [i for i, value in enumerate(batch.non_tensor_batch["uid"])
                                    if value == uid
                                ]].sum(-1).detach().cpu().tolist())
                                for uid in dict.fromkeys(batch.non_tensor_batch["uid"].tolist())
                            ]))
                            metrics.update(jass_metrics(batch))
                            combined_gen_output = None
                            timing_raw.setdefault("gen", 0.0)
                        elif curr_step_profile:
                    ...
                    with marked_timer("reward", timing_raw, color="yellow"):
                        ...
                        if agentic_bndsr:
                            ...
                            batch = self._apply_b_ndsr(batch, metrics)
                        reward_tensor, judge_metrics = apply_judge_reward(reward_tensor, batch)
